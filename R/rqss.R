...
#       o  ...
#
   stopifnot(requireNamespace("interp"))
#
    y <- x[,2]
    x <- x[,1]
    n <- length(x)
...
function (x, render = "contour", ncol = 100, zcol = NULL, ...)
{
    ...
    if (render == "rgl") {
        ...
        colz <- collut[colz]
        s <- c(t(interp::triangles(trinew)[, 1:3]))
        ...
        if(render == "contour"){
                ...
                interp::convex.hull(trinew, plot.it = TRUE, add = TRUE)
                }
...
if(length(dummies)){
	if (is.list(dummies)){
        ...
        Tnew <- interp::tri.mesh(x,y)
        T <- interp::triSht2tri(Tnew)
        s <- interp::in.convex.hull(Tnew,xd,yd)
        x <- c(x,xd[s])
        y <- c(y,yd[s])
        ndum <- sum(s)
        zdum <- as.matrix.csr(0,n,ndum)
...
"triogram.penalty" <- function (x, y, eps = .Machine$double.eps)
{
    ...
    trinew <- interp::tri.mesh(x, y)
    tri <- interp::triSht2tri(trinew)
    bnd <- interp::on.convex.hull(trinew,x,y)
    q <- length(tri$tlist)
    m <- 13 * n
...
predict.qss2 <- function (object, newdata, ...) 
{
    ...
    trinew <- interp::tri.mesh(x, y)
    tri <- interp::triSht2tri(trinew)
    if (!all(interp::in.convex.hull(trinew, newx, newy))) 
